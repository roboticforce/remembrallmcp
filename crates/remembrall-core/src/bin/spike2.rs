...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter("remembrall=warn,warn")
        .init();

    let database_url = std::env::var("DATABASE_URL")
        .unwrap_or("postgres://postgres:postgres@localhost:5450/remembrall".into());

    println!("=== RemembrallMCP Spike 2: Multi-Project Codebase Indexing ===\n");
