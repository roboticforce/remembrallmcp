//! Spike 1: Validate RemembrallMCP core against real Postgres + pgvector.
//! Run: cargo run --bin spike

use remembrall_core::config::Config;
use remembrall_core::graph::store::GraphStore;
use remembrall_core::graph::types::*;
use remembrall_core::memory::store::MemoryStore;
use remembrall_core::memory::types::*;
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter("remembrall=debug,info")
        .init();

    let database_url =
        std::env::var("DATABASE_URL").unwrap_or("postgres://postgres@localhost:5450/remembrall".into());

    println!("=== RemembrallMCP Spike 1: Core Validation ===\n");
    ...
    let schema = "remembrall".to_string();
    ...
    let fingerprint_check = memory_store
        .find_by_fingerprint(&remembrall_core::memory::store::compute_fingerprint_pub(&memories[0].content))
        .await?;
