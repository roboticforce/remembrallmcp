...
async fn main() -> Result<()> {
    let database_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgres://postgres:postgres@localhost:5450/remembrall".to_string());
    ...
    let schema = "remembrall_integration_test";

    println!("=== RemembrallMCP Integration Tests ===");
